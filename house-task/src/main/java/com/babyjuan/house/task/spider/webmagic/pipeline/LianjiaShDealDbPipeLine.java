...
public class LianjiaShDealDbPipeLine implements Pipeline {
    ...
    public void process(ResultItems resultItems, Task task) {
        ...
        List<ShHouseDeal> oldDeals = mysqlRepository.selectShHouseDeal(currentDeal.getHouseCode());
        if (oldDeals.isEmpty()) {
    ...
    private void updateOld(ShHouseDeal oldDeal) {
        mysqlRepository.updateShHouseDeal(oldDeal);
    }

    private void insertNew(ShHouseDeal currentDeal) {
        mysqlRepository.insertShHouseDeal(currentDeal);
    }
